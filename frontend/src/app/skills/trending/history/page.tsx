...
function WeekSection({ week }: { week: GitHubTrendingWeek }) {
  return (
    <div className="mb-12">
...
export default function TrendingHistoryPage() {
  const archivedWeeks = getArchivedTrending();
  const [currentPage, setCurrentPage] = useState(1);
  const WEEKS_PER_PAGE = 3;

  const totalPages = Math.ceil(archivedWeeks.length / WEEKS_PER_PAGE);
  const startIndex = (currentPage - 1) * WEEKS_PER_PAGE;
  const displayedWeeks = archivedWeeks.slice(startIndex, startIndex + WEEKS_PER_PAGE);
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        >
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-white">往期热门</h1>
              <p className="text-sm text-gray-500">GitHub 热门项目历史归档</p>
            </div>
          </div>
          <p className="text-gray-400 max-w-2xl">
            回顾过去每周的 GitHub 热门 AI 项目，追踪技术发展趋势。
          </p>
        </motion.div>
          ...
          {archivedWeeks.length > 0 ? (
            <>
              {displayedWeeks.map((week) => (
                <WeekSection key={`${week.year}-${week.weekNumber}`} week={week} />
              ))}
            ...
            <div className="text-center py-20">
              ...
              <h3 className="text-lg font-medium text-white mb-2">暂无历史数据</h3>
